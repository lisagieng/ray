I plan on making separate XML entries for each object which we have on-file. Its attributes are as follows:
...
3) JS file searches through XML file for entry with name that matches text
4) JS retrieves matching XML file (or prints error if no such entry exists)
